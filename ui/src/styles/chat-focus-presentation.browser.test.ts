...
const FIXTURE = `
  ...
  <div style="position: relative; height: 80px; margin-left: 32px">
    <button class="chat-position-rail__marker" style="--chat-position-marker: 50%" type="button" aria-label="Message position" data-focus-case="position marker">
      <span class="chat-position-rail__dot" aria-hidden="true"></span>
    </button>
    <div class="chat-position-rail__preview" style="--chat-position-preview: 50%">Message preview</div>
  </div>
  ...
  <span id="border-reference" style="color: color-mix(in srgb, var(--ring) 50%, var(--border))"></span>
`;
...
describe("chat focus presentation", () => {
  ...
  async function checkKeyboardOutlines(forcedColors = false) {
    ...
    const preview = getComputedStyle(container.querySelector(".chat-position-rail__preview")!);
    expect.soft(preview.borderTopStyle).toBe("solid");
    expect.soft(preview.borderTopWidth).toBe("1px");
    if (!forcedColors) {
      expect
        .soft(preview.borderTopColor)
        .toBe(getComputedStyle(container.querySelector("#border-reference")!).color);
  ...
  it.each(THEMES)("paints keyboard outlines and the preview border in %s", async (theme, mode) => {
    setTheme(theme, mode);
