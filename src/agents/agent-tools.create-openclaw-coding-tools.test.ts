...
describe("createOpenClawCodingTools", () => {
  ...
  it("forwards prepared run facts to plugin-only tool construction", () => {
    ...
    try {
      ...
      expect(resolvePluginToolsSpy.mock.calls[0]?.[0].currentTurnDelivery?.toolRef).toEqual({});
    } finally {
  ...
  it("keeps only the exact current-turn delivery instance through restrictive policy", () => {
    ...
    const currentTurnDeliveryToolRef: NonNullable<
      NonNullable<Parameters<typeof createEmbeddedAttemptCodingTools>[1]>["toolRef"]
    > = {};
    const resolvePluginToolsSpy = vi
      .spyOn(openClawPluginTools, "resolveOpenClawPluginToolsForOptions")
      .mockImplementation((params) => {
        if (params.currentTurnDelivery?.toolRef) {
          params.currentTurnDelivery.toolRef.value = currentTurnTool;
        }
