    ...
    it(
      ...
      async () => {
        ...
        for (const [index, turn] of turns.entries()) {
          ...
          await waitFor("current-turn outbound", () => {
          ...
          const history = await waitFor("current completed exec transcript result", async () => {
            ...
            const completedExecs = payload.messages.filter(
              (message) =>
                isRecord(message) &&
                message.role === "toolResult" &&
                message.toolName === "exec" &&
                JSON.stringify(message).includes('"status":"completed"'),
            );
            const last = payload.messages.at(-1);
            return completedExecs.length === index + 1 &&
              isRecord(last) &&
          ...
          expect(settled.requests[index]).toMatchObject({ plannedToolName: "exec" });
